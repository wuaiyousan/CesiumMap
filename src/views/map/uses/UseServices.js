 ...
 * @LastEditTime: 2024-07-10 16:38:02
 * @Description: UseServices
 */
...
export default function UseServices() {
  ...
  // 获取接口数据
  function getBizList(info) {
    let { type, url, params, dataType, extData: {response} } = info
    if (response) {
    ...
    // todo:跨域问题,nginx配置
    if (theType === 'GET') {
      if(dataType === 'featuresmap'){
        return http.getJson(`${url}${http.formatParams(params)}`)
      }else {
        return http.get(`${url}${http.formatParams(params)}`)
      } 
  ...
  // 接口返回数据结构处理
  async function getFeatureList(info) {
    let list = await getBizList(info);
